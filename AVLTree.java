 ...
 class LUC_AVLTree {
     ...
     */
     ...
     private Node deleteElement(int value, Node node) {
        ...
        // Traversion
        if (value < node.value) {
            node.leftChild = deleteElement(value, node.leftChild); // recursively traverse left if current node.value is greater than value
        } else if (value > node.value) {
            node.rightChild = deleteElement(value, node.rightChild); // recursively traverse rihgt if current node.value is less than value
        } else {
            // otherwise, we are at the node to be deleted (NTBD)
            // if NTBD has 0 children, return null to remove node
            if (node.leftChild == null && node.rightChild == null) { 
                return null;
            }
    
            // if NTBD has 1 child, determine which it is and replace it with that child
            if (node.leftChild == null) {
                return node.rightChild; // node is replaced with its right child
            }
            else if (node.rightChild == null) {
                return node.leftChild; // node is replaced with its left child
            }
    
            // if NTBD has 2 children, replace NTBD with the in-order-successor in right child's sub-tree
            else {
            Node successor = minValueNode(node.rightChild);
            node.value = successor.value;
            node.rightChild = deleteElement(successor.value, node.rightChild); // delete the successor node, as it remains in the tree
            }
        }

        // After traversion, Rotation
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
        // check balanceFactor to see if a rotation of the nodes is necessary
        int bf = getBalanceFactor(node);
        if (bf > 1) {
            if (getBalanceFactor(node.leftChild) >= 0) { // LL rotation if leftChild's bf is positive
                return LLRotation(node);
            } else { // otherwise, LR rotation
                return LRRotation(node);
            }
        }
        if (bf < -1) {
            if (getBalanceFactor(node.rightChild) >= 0) {
                return RLRotation(node); // RL rotation if leftChild's bf is positive
            } else {
                return RRRotation(node); // RR rotation otherwise
            }
        }
